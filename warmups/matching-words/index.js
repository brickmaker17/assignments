//seperate all the words into an array
//loop through the array
//for each string, check if it appears elsewhere in the array;
//if so, check if its not laready in output array, and push it there
let matchWords = (str) => {
    let output = [];
    let arr = str.toLowerCase().str.split("");
    // console.log(arr);
    for (let i = 0; i < arr.length; i++) {
        let currentWord = arr.splice(i, 1);
